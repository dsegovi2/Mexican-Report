...
housing_ddi3 <- usa_ddi %>% 
  select(YEAR, MULTYEAR, RACE, RACED, CITY, PUMA, HISPAN, PERNUM, PERWT, HHWT, SERIAL, RENTGRS, HHINCOME) %>% 
  clean_names() 
...
rent_burdened = housing_ddi3 %>% 
  ...
  as_survey_design(weights = hhwt) %>% 
  group_by(race_ethnicity) %>% 
  summarise(income_mean = survey_mean(hhincome, na.rm = T),
            rentgrs_mean = survey_mean(rentgrs, na.rm = T),
            rent_income_mean = survey_mean(rent_income, na.rm = T))  
...
rent_burdened = housing_ddi3 %>% 
  ...
  mutate(race_ethnicity = case_when(hispan ==0 & race == 1 ~ "White (non-Hispanic or Latino)",
                                    ...
                                    TRUE ~ NA_character_)) %>% 
  as_survey_design(weights = hhwt) %>% 
  survey_count( race_ethnicity, rentgrs, hhincome, name="count")  
```
