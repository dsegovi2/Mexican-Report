...
table_3.b2_2 <- table_3.b2 %>% 
  ...
  kbl(booktabs = T,
      col.names = c("Race/Ethnicity", "Number", "Percent" , "Number", "Percent" ),
      align = c("l",rep("c",4)),
  ...
  # column_spec(1, width = "10em") %>%
  # column_spec(2:8, width = "3.8em") %>%
  add_header_above( c("Race/Ethnicity", "More than 30%" = 2, "More than 50%" = 2),
                    # background = "#cbc9e2",
                    bold = T, font_size =8) %>%
  # row_spec(0, bold = T,
  #          # background = "#f2f0f7",
  ...
  footnote(general = "Source: IPUMS-USA database (2018-2022). Tabulations by Great Cities Institute\nRent Burdened Households Defined as Spending 30% or More of Income on Rent",
           general_title = "")  
...
cost_burdened = housing_ddi3 %>% 
  ...
  mutate(race_ethnicity = case_when(hispan ==0 & race == 1 ~ "White (non-Hispanic or Latino)",
                                    ...
                                    hispan %in% c(2,3,4) ~ "Hispanics other than Mexican",
                                    hispan ==1 ~ "Mexican", 
                                    hispan ==0 & race %in%c(3:9) ~ "Other Latinos",
                                    TRUE ~ NA_character_)) %>% 
