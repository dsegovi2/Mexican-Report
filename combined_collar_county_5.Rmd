...
table_1d.2.1 <- income_levels %>% select(race_ethnicity, avg_incwage , weighted_median_incwage) %>% 
  ...
  c(
    ...
    "\\footnotesize{Data covers full-time employees working a minimum of 35 hours per week for at least 48 weeks each year.}"
  ),
  notation = "none",
  threeparttable = TRUE,
  escape = FALSE
...
table_1d.2.2 <- income_levels %>% select(race_ethnicity, avg_incwage , weighted_median_incwage) %>% 
  ...
  c(
    "\\footnotesize{Data Source: IPUMS USA, University of Minnesota, www.ipums.org (2018-2022 American Community Survey 5-year Estimates). Tabulations by Great Cities Institute.}",
    "\\footnotesize{The data includes all employed individuals, regardless of whether they work full-time or part-time.}"
  ),
  notation = "none",
  threeparttable = TRUE,
  escape = FALSE
...
df_latino_city <- data.frame(
  commnty = "Hispanic or Latino City Mean",
...
row_spec(0, bold = TRUE, font_size = 8, align = "c") %>%
  ...
  c(
    ...
    "\\footnotesize{Lung cancer is defined as lung and bronchus cancer for ages 15 and over, risk-adjusted.}"
...
business_ownership %>%
  ...
  geom_bar(position=position_dodge(width=0.7), stat = "identity",
           ...
           width=0.7) +
  labs(y="", x = "",
       title="Business count by owner demographic group in Chicago-Naperville-Elgin,\nIL-IN-WI Metro Area",
       caption = "Data Sources: Annual Business Survey, 2021 (Census); Nonemployer, Statistics by Demographics, 2021 (Census),\nTabulations by Great Cities Institute.",
       fill="") +
...
msa_table_ownership_income <- read_csv("msa_table_ownership_income.csv")

county_table_ownership_income <- read_csv("county_table_ownership_income.csv")
```


# 1: Income and Home Ownership for Mexicans by 10 Metropolitan Areas with the Largest Share of Mexican Population, 2018-2022 (ACS 5-year Estimates)
...
table_1 <- msa_table_ownership_income %>% select(metro_name, total_pop, total_mexican,  prc_mexican, avg_incwage, weighted_median_incwage, per2) %>% 
  ...
  kbl(, booktabs = T,
      ...
      align = c("l",rep("r",6)),
      caption = "Income and Home Ownership for Mexicans by 10 Metropolitan Areas with the Largest Share of Mexican Population, 2018-2022 (ACS 5-year Estimates)",
      format.args = list(big.mark = ","),
      linesep = "")  %>% 
  kable_classic(full_width = F,  
                font_size = 8,
...
# 2: Income and Home Ownership for Mexicans by 10 Counties with the Largest Share of Mexican Population, 2018-2022 (ACS 5-year Estimates)
...
table_2 <- county_table_ownership_income %>% select(county_name, total_pop, total_mexican,  prc_mexican, avg_incwage, weighted_median_incwage, per2) %>% 
  ...
  kbl(, booktabs = T,
      ...
      align = c("l",rep("r",6)),
      caption = "Income and Home Ownership for Mexicans by 10 Counties with the Largest Share of Mexican Population, 2018-2022 (ACS 5-year Estimates)",
      format.args = list(big.mark = ","),
      linesep = "")  %>% 
  kable_classic(full_width = F,  
                font_size = 8,
