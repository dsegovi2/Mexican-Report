...
top_occ_mxc <- occupation4 %>%
  filter(race_ethnicity == "Mexican") %>%
  arrange(desc(count)) %>%
  slice(1:10) %>%
  pull(occ)

top_occ_latinos <- occupation4 %>%
...
occupation5 <- occupation4 %>% 
  filter(occ %in% top_occ_mxc & race_ethnicity == "Mexican" | occ %in% top_occ_latinos & race_ethnicity == "Other Latinos") %>% 
  
  arrange(factor(race_ethnicity, levels = levels(occupation4$race_ethnicity)), desc(count)) %>% 
  ...
  kable(booktabs = T,
      ...
      align = c("l","l" ,rep("c",2)),
      caption = "The Top 10 Occopations where Mexicans and Other Latinos have the highest employment in Chicago, 2018-2022",
      format.args = list(big.mark = ","),
      linesep = "") %>%
  kable_classic(full_width = F,
                font_size = 8,
