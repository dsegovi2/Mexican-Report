...
##### race/pop data for white/black pop

nhgis_csv_file <- "C:/Users/dsegovi2/Box/Great Cities Institute/Research/Mexican Report/nhgis0054_csv.zip"
nhgis_shp_file <- "C:/Users/dsegovi2/Box/Great Cities Institute/Research/Mexican Report/nhgis0054_shape.zip"

# merge sf with df
extract_2018_2022_df <- read_nhgis(nhgis_csv_file, file_select = matches("nhgis0054_ds262_20225_tract"), verbose = TRUE)
extract_2018_2022_sf <- read_ipums_sf(nhgis_shp_file, file_select = matches("nhgis0054_shapefile_tl2022_us_tract_2022.zip"), verbose = FALSE)
extract_2018_2022 <- ipums_shape_full_join(extract_2018_2022_df, extract_2018_2022_sf, by = "GISJOIN")
...
tract_pop_2018_2022 <- extract_2018_2022 %>% 
  filter(STATE == "Illinois" & COUNTY == "Cook County") %>% 
  select(GISJOIN, YEAR, STUSAB, STATE, COUNTY,TL_GEO_ID, TRACTA, NAME_E, AQNOE001, AQNOE003, AQNOE004) %>% 
  group_by(GISJOIN, YEAR, TL_GEO_ID) %>% 
  mutate(total_pop = sum(AQNOE001, na.rm = T),
         total_nh_white = sum(AQNOE003, na.rm = T),
         prc_nh_white = 100* round(total_nh_white /total_pop, 4),
          total_nh_black = sum(AQNOE004, na.rm = T),
         prc_nh_black = 100* round(total_nh_black/total_pop, 4)) %>% 
  st_transform(4326)
...
chicago_ca_2018_2022_df <- tract_2018_22_ca_point %>% 
  group_by(community) %>% 
  summarise(total_pop = sum(AQNOE001, na.rm = T),
         total_nh_white = sum(AQNOE003, na.rm = T),
         prc_nh_white = 100* round(total_nh_white /total_pop, 4),
          total_nh_black = sum(AQNOE004, na.rm = T),
         prc_nh_black = 100* round(total_nh_black/total_pop, 4)) %>% 
...
# merge mexican with nh_black and nh_white pop


chicago_ca_2018_22_sf <- chicago_ca_2018_22_sf %>% left_join(chicago_ca_2018_2022_df)
