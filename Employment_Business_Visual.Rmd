...
table_5a.2 <-  industry %>%
  ...
  select(industry, count,  prc2) %>%
   ...
   # using kable
  kable(booktabs = T,
        col.names = c("Industry", "Number", "% Share"  ),
      align = c("l",rep("c",2)),
...
# ipums_val_labels(cps_ddi$hispan)
...
Union <- cps_ddi %>% 
  ...
  summarise(unionized = sum(count),
            prc = 100*round(unionized/total_employed, 4),
            prc2 = paste0(prc, "%")) %>% 
  distinct()


table5b <- Union %>% 
  select(race_ethnicity,total_employed,  unionized, prc2) %>% 
  # using kable

  kbl(booktabs = T,
      col.names = c("Race/Ethnicity",  "Total Employed" ,"Unionized Workers", "Percent"),
      align = c("l",rep("c",3)),
      caption = " Numbers and percentages of unionized Mexican workers by Race/Ethnicity in Chicago, 2022",
      format.args = list(big.mark = ","),
      linesep = "") %>%
 kable_classic(full_width = F,
                font_size = 8,
