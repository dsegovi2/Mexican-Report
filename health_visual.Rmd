...
df_health <- chicago_ca_pop %>% arrange(desc(prc)) %>%   slice_max(prc, n = 10) %>% select(commnty, prc) %>% mutate(commnty = str_to_title(tolower(commnty))) %>% left_join(chicago_health, by = c("commnty" = "name")) %>% select(-layer) %>%
 mutate(across(matches("rate$|crimes$"), ~ round(as.numeric(.), 1))) %>%
 mutate(across(matches("rate$|prc$"), ~ ifelse(is.na(.), "missing data", paste0(., "%")))) %>% mutate(across(ends_with("crimes"), ~ ifelse(is.na(.), "missing data", paste0(., " crimes"))))
...
```{r}
# visuals
table_4a <- df_health  %>% select(commnty, prc, obesity_rate, diabetes_rate, asthma_rate, lbw_rate)   %>% 
 
   # using kable 
  
  kbl(, booktabs = T,
      col.names = c("Community Area", "Percent Mexican", "Adult Obesity Rate", "Adult Diabetes Rate", "Adult Asthma Rate", "Low Birthweight Rate"),
      align = c("l",rep("c",5)),
...
```{r}
# visuals
table_4b <- df_rates  %>% select(race_ethnicity, weighted_n_without_insurance, no_insurance_rate)   %>% 
...
```{r}
# visuals
table_4d <- df_health  %>% select(commnty, prc, drinking_rate, smoking_rate, fruit_veg_rate)   %>% 
 
   # using kable 
  
  kbl(, booktabs = T,
      col.names = c("Community Area", "Percent Mexican", "Adult Binge Drinking Rate", "Adult Smoking Rate", "Adult Healthy Diet Rate"),
      align = c("l",rep("c",4)),
      caption = "Health Behaviors, 2022-2023 and 2017-2021",
      format.args = list(big.mark = ","),
      linesep = "")  %>% 
  kable_classic(full_width = F,  
                font_size = 8,
...
```{r}
# visuals
table_4e <- df_health  %>% select(commnty, prc, sexual_assault_crimes, unmet_mental_health_rate)   %>% 
 
   # using kable 
  
  kbl(, booktabs = T,
      col.names = c("Community Area", "Percent Mexican", "Criminal Sexual Assault", "Unmet Mental Health Need Rate"),
      align = c("l",rep("c",3)),
      caption = "Social and Public Health, 2022-2023",
      format.args = list(big.mark = ","),
      linesep = "")  %>% 
  kable_classic(full_width = F,  
                font_size = 8,
