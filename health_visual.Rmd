...
chicago_health <- read_csv("Health Data/Chicago Health Atlas Data Download - Community areas.csv") %>% clean_names() %>% rename(
  ...
  sexual_assault_rate =  czs_2023, 
  unmet_mental_health_rate = hcsumhap_2022_2023
)

# merge chicago ca with health atlas

df_health <- chicago_ca_pop %>% arrange(desc(prc)) %>%   slice_max(prc, n = 10) %>% select(commnty, prc) %>% mutate(commnty = str_to_title(tolower(commnty))) %>% left_join(chicago_health, by = c("commnty" = "name"))
```
