...
# mutate(across(matches("rate"), ~ ifelse(is.na(.), "missing data", paste0(., "%")))) %>%
  #mutate(across(ends_with("crimes"), ~ ifelse(is.na(.), "missing data", paste0(., " crimes"))))
...
mexican_average_row <- mexican_top %>%
  summarise(
    ...
    obesity_rate = mean(obesity_rate, na.rm = TRUE),
    diabetes_rate = mean(diabetes_rate, na.rm = TRUE),
    asthma_rate = mean(asthma_rate, na.rm = TRUE),
    lbw_rate = mean(lbw_rate, na.rm = TRUE)
  )
...
```{r}
# visuals
table_4d <- df_health  %>% select(commnty, prc_mxc, drinking_rate, smoking_rate, fruit_veg_rate)   %>% 
...
```{r}
# visuals
table_4e <- df_health  %>% select(commnty, prc_mxc, sexual_assault_crimes, unmet_mental_health_rate)   %>% 
