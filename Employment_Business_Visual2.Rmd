...
industry3 <- industry2 %>% 
  ...
  group_by(race_ethnicity) %>% 
  mutate(sub_industry = case_when(ind == 8680 ~ "Accommodation and Food Services: Restaurants and other food services",
                    ind == 770 ~ "Construction (the cleaning of buildings and dwellings is incidental during construction and immediately after construction)",
                    ind == 7860 ~ "Educational Services: Elementary and secondary schools",
                    ind == 8191 ~ "Health Care and Social Assistance: General medical and surgical hospitals, and specialty (except psychiatric and substance abuse) hospitals",
                    ind == 7690 ~ "Administrative and support and waste management services: Services to buildings and dwellings (except cleaning during construction and immediately after construction)",
                    ind == 4971 ~ "Retail Trade: Supermarkets and other grocery (except convenience) stores",
                    ind == 8770 ~ "Other Services, Except Public Administration: Automotive repair and maintenance",
                    ind == 7870 ~ "Educational Services: Colleges, universities, and professional schools, including junior colleges",
                    ind == 7770 ~ "Administrative and support and waste management services: Landscaping services",
                    ind == 3990 ~ "Manufacturing: Not specified manufacturing industries"
...
table_5a.2_2 <-  industry3 %>%
  filter(race_ethnicity == "Mexican") %>% 
  # filter(race_ethnicity %in% c("Other Latinos")) %>% 
  select(sub_industry, count,  prc2) %>%

   # using kable
  kable(booktabs = T,
        col.names = c("Sub Industry", "Number", "% Share of Employed"   ),
      align = c("l",rep("c",2)),
...
## 5B: Numbers and percentages of unionized Mexican workers
